...
void loop()
{
    ...
    if (app.ready() && !taskComplete)
    {
        ...
        // Push int
        Database.push<int>(aClient, "/test/int", 12345, asyncCB, "push_int");

        // Push bool
        Database.push<bool>(aClient, "/test/bool", true, asyncCB, "push_bool");

        // Push string
        Database.push<String>(aClient, "/test/string", "hello", asyncCB, "push_string");

        // Push json
        Database.push<object_t>(aClient, "/test/json", object_t("{\"data\":123}"), asyncCB, "push_json1");
        ...
        Database.push<object_t>(aClient, "/test/json", json, asyncCB, "push_json2");
        ...
        // Push array
        Database.push<object_t>(aClient, "/test/arr", arr, asyncCB, "push_array");

        // Push float
        Database.push<number_t>(aClient, "/test/float", number_t(123.456, 2), asyncCB, "push_float");

        // Push double
        Database.push<number_t>(aClient, "/test/double", number_t(1234.56789, 4), asyncCB, "push_double");
    }
}
